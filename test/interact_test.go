...
func TestFavorite(t *testing.T) {
	...
	feedResp.Value("status_msg").String().IsEqual("success") // 确保响应状态码为 0，表示成功
	...
	favoriteListResp := e.GET("/douyin/favorite/list/").
		...
		Status(http.StatusOK).
		JSON().Object()
	//favoriteListResp.Value("status_msg").String().Equal("success") // 确保获取收藏列表成功
...
func TestComment(t *testing.T) {
	...
	addCommentResp := e.POST("/douyin/comment/action/").
		...
		Status(http.StatusOK).
		JSON().Object()
	addCommentResp.Value("status_msg").String().IsEqual("success")      // 确保添加评论操作成功
	...
	commentListResp := e.GET("/douyin/comment/list/").
		...
		Status(http.StatusOK).
		JSON().Object()
	commentListResp.Value("status_msg").String().IsEqual("success") // 确保获取评论列表成功
	...
	delCommentResp := e.POST("/douyin/comment/action/").
		...
		Status(http.StatusOK).
		JSON().Object()
	delCommentResp.Value("status_msg").String().IsEqual("success") // 确保删除评论操作成功
}
