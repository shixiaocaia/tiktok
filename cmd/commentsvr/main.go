...
func Init() {
	...
	if err := config.Init(); err != nil {
		log.Fatalf("init commentSvr config failed, err:%v\n", err)
	}
	// 初始化日志
	log.InitLog()
...
func Run() error {
	...
	// 注册grpc server
	pb.RegisterCommentServiceServer(server, &service.CommentService{})
	// 注册服务健康检查
	...
	// 启动
	log.Infof("TikTokLite.commentSvr listening on %s:%d", config.GetGlobalConfig().SvrConfig.Host, config.GetGlobalConfig().SvrConfig.Port)
	go func() {
